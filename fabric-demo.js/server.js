...
app.get('/fhir/:resource_type', (req, res) => {
    req.body = { "fcn": "queryResource", "args": [ req.query.id, resource_type ] }
    sendTransactionHTTP(req, res)
})

// POST the record using the JSON in the request body
app.post('/fhir/:resource_type', (req, res) => {
    req.body = { "fcn": "postResource", "args": [ req.body.id, req.body ] }
    sendTransactionHTTP(req, res)
})

// POST the configuration for the NATS and FHIR external servers
app.post('/config', (req, res) => {
    req.body = { "fcn": "configureContract", "args": [ req.body ] }
    sendTransactionHTTP(req, res)
})

// POST the configuration for the NATS and FHIR external servers
app.post('/eligibility', (req, res) => {
    req.body = { "fcn": "checkEligibility", "args": [ req.body.id, req.body ] }
    sendTransactionHTTP(req, res)
})

/*
 * Send the transaction originating from a REST call. 
 */
async function sendTransactionHTTP (req, res) {
    ...
    try {
        ...
        .then(respbytes => {
            ...
            console.log(`Submitted a ${fcn} transaction`)
        })
        .catch(error => {
...
/*
 * Send the transaction originating from a NATS message. 
 */
